...
#include <asr_sdm_vio/backend/graph_manager.h>
...
BackendInterface::Ptr makeBackend(const rclcpp::Node::SharedPtr& node)
{
  ...
  try {
    default_trace_dir = ament_index_cpp::get_package_share_directory("asr_sdm_ros2") + std::string("/trace");
  } catch (...) {
    default_trace_dir = std::string("/tmp/asr_sdm_trace");
  }
