...
public:
  explicit BenchmarkNode(const PipelineType & type, const rclcpp::Node::SharedPtr & node);
  ...
  bool loadNextImages(
    const std::string & dataset_basedir, std::ifstream & img_fs, double * stamp_sec,
...
BenchmarkNode::BenchmarkNode(const PipelineType & type, const rclcpp::Node::SharedPtr & node)
: SvoInterface(type, node)
{
  CHECK(node);
  ...
  img_noise_sigma_ = vk::param<double>(node, "dataset_noise_sigma", 0.0);
  sleep_us_ = static_cast<size_t>(vk::param<int>(node, "sleep_us", 0));

  first_frame_id_ = static_cast<size_t>(vk::getParam<int>(node, "svo/dataset_first_frame", 0));
  last_frame_id_ = static_cast<size_t>(vk::getParam<int>(node, "svo/dataset_last_frame", 0));

  trace_only_kf_ = vk::getParam<bool>(node, "svo/trace_only_keyframes", false);

  blackout_start_id_ = static_cast<size_t>(vk::getParam<int>(node, "svo/blackout_first_id", 0));
  blackout_end_id_ = static_cast<size_t>(vk::getParam<int>(node, "svo/blackout_last_id", 0));
}
...
void BenchmarkNode::traceIdTimePose(
  const Transformation & pose, const size_t id, const double time_sec)
{
...
void BenchmarkNode::traceSpeedBias(
  const svo::ViNodeState & state, const size_t id, const double time_sec)
{
...
bool BenchmarkNode::loadNextImages(
  const std::string & dataset_basedir, std::ifstream & img_fs, double * stamp_sec, size_t * img_id,
  std::vector<cv::Mat> * images)
{
  ...
  if (
    (*img_id < first_frame_id_) || (last_frame_id_ > 0 && *img_id >= last_frame_id_) ||
    img_fs.eof()) {
    img_fs.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
    return false;
  }
  ...
  while (img_fs.good() && !img_fs.eof() && rclcpp::ok()) {
    ...
    if (svo_->stage() == Stage::kTracking || svo_->getBundleAdjuster()) {
      ...
      if (!trace_only_kf_ || nframe.isKeyframe()) {
        ...
        if (
          svo_->isBackendScaleInitialised() &&
...
void BenchmarkNode::runBlenderBenchmark(const std::string & dataset_dir, bool depthmap_has_zbuffer)
{
  ...
  if (
    trace_trans_error_.fail() || trace_rot_error_.fail() || trace_depth_error_.fail() ||
    trace_gt_pose_.fail())
  ...
  if (dataset.getNextFrame(n_pyr_levels, ref_frame, &depthmap)) {
    ...
    for (size_t i = 0; i < ref_frame->num_features_; ++i) {
      ...
      if (depthmap_has_zbuffer) {
        ...
        SVO_DEBUG_STREAM(
          "Depth (" << ref_frame->px_vec_(0, i) << " , " << ref_frame->px_vec_(1, i)
                    << ") = " << depth);
  ...
  while (dataset.getNextFrame(n_pyr_levels, cur_frame, nullptr) && rclcpp::ok()) {
    ...
    publishResults({cur_frame->img()}, getNode()->get_clock()->now().nanoseconds());
    ...
    if (svo_->getLastFrames() && svo_->isBackendScaleInitialised()) {
      ...
      traceIdTimePose(
        svo_->getLastFrames()->at(0)->T_f_w_.inverse(), static_cast<size_t>(cur_frame->id()),
...
void BenchmarkNode::runKittiBenchmark(const std::string & dataset_dir)
{
  ...
  while (dataset_fs.good() && !dataset_fs.eof() && rclcpp::ok()) {
    ...
    publishResults({img_l_8uC1, img_r_8uC1}, getNode()->get_clock()->now().nanoseconds());
...
void BenchmarkNode::runArrayBenchmark(const std::string & dataset_dir)
{
  ...
  size_t first_frame_id = static_cast<size_t>(vk::getParam<int>(getNode(), "svo/dataset_first_frame", 0));
  while (img_fs.good() && !img_fs.eof() && rclcpp::ok()) {
...
int main(int argc, char ** argv)
{
  ...
  auto node = std::make_shared<rclcpp::Node>("svo_benchmark");

  std::string benchmark_dir(vk::param<std::string>(node, "dataset_directory", "/tmp"));
  if (vk::param<bool>(node, "dataset_is_kitti", false)) {
    svo::BenchmarkNode benchmark(svo::PipelineType::kStereo, node);
    benchmark.runKittiBenchmark(benchmark_dir);
  } else if (vk::param<bool>(node, "dataset_is_blender", false)) {
    const bool depthmap_has_zbuffer = vk::param<bool>(node, "depthmap_has_zbuffer", false);
    svo::BenchmarkNode benchmark(svo::PipelineType::kMono, node);
    benchmark.runBlenderBenchmark(benchmark_dir, depthmap_has_zbuffer);
  } else {
    if (vk::param<bool>(node, "dataset_is_stereo", true)) {
      svo::BenchmarkNode benchmark(svo::PipelineType::kStereo, node);
