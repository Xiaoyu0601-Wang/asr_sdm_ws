...
void publishLineList(
  ...
  const Eigen::Vector3f & color, const double scale, const double alpha = 1.0)
{
...
void publishPositionVecAsPC(
  ...
  const std::vector<rpg::PositionVec> & vec_of_position_vec, const std::vector<float> & intensities)
{
  CHECK_EQ(vec_of_position_vec.size(), intensities.size());

  svo::Visualizer::PointCloud pc;
  pcl_conversions::toPCL(node->get_clock()->now(), pc.header.stamp);
...
Visualizer::Visualizer(
  const std::string & trace_dir, const rclcpp::Node::SharedPtr & node, const size_t n_cameras)
: node_(node),
  trace_id_(0),
  ...
  pc_(new PointCloud),
  publish_world_in_cam_frame_(vk::param<bool>(node_, "publish_world_in_cam_frame", true)),
  publish_map_every_frame_(vk::param<bool>(node_, "publish_map_every_frame", false)),
  publish_points_display_time_(
    rclcpp::Duration::from_seconds(vk::param<double>(node_, "publish_point_display_time", 0))),
  publish_seeds_(vk::param<bool>(node_, "publish_seeds", true)),
  publish_seeds_uncertainty_(vk::param<bool>(node_, "publish_seeds_uncertainty", false)),
  publish_active_keyframes_(vk::param<bool>(node_, "publish_active_kfs", false)),
  trace_pointcloud_(vk::param<bool>(node_, "trace_pointcloud", false)),
  vis_scale_(vk::param<double>(node_, "publish_marker_scale", 1.2))
{
  pub_frames_ =
    node_->create_publisher<visualization_msgs::msg::Marker>("keyframes", rclcpp::QoS(10));
  pub_points_ =
    node_->create_publisher<visualization_msgs::msg::Marker>("points", rclcpp::QoS(10000));
  pub_imu_pose_ = node_->create_publisher<geometry_msgs::msg::PoseWithCovarianceStamped>(
    "pose_imu", rclcpp::QoS(10));
  pub_info_ = node_->create_publisher<asr_sdm_msgs::msg::Info>("info", rclcpp::QoS(10));
  pub_markers_ =
    node_->create_publisher<visualization_msgs::msg::Marker>("markers", rclcpp::QoS(100));
  pub_pc_ = node_->create_publisher<sensor_msgs::msg::PointCloud2>("pointcloud", rclcpp::QoS(1));
  ...
  for (size_t i = 0; i < n_cameras; ++i) {
    pub_dense_.at(i) = node_->create_publisher<asr_sdm_msgs::msg::DenseInputWithFeatures>(
      "dense_input/cam" + std::to_string(i), rclcpp::QoS(2));
    pub_images_.at(i) = it.advertise("image/cam" + std::to_string(i), 10);
    pub_cam_poses_.at(i) = node_->create_publisher<nav_msgs::msg::Odometry>(
      "pose_cam/cam" + std::to_string(i), rclcpp::QoS(10));
  }
  ...
  pub_loop_closure_ = node_->create_publisher<visualization_msgs::msg::MarkerArray>(
    "loop_closures", rclcpp::QoS(10));
  pub_pose_graph_ = node_->create_publisher<visualization_msgs::msg::MarkerArray>(
    "pose_graph", rclcpp::QoS(10));
  pub_pose_graph_map_ =
    node_->create_publisher<sensor_msgs::msg::PointCloud2>("pose_graph_pointcloud", rclcpp::QoS(10));
#endif

#ifdef SVO_GLOBAL_MAP
  pub_global_map_kfs_opt_ = node_->create_publisher<visualization_msgs::msg::MarkerArray>(
    "global_map_kfs", rclcpp::QoS(10));
  pub_global_map_query_kfs_ = node_->create_publisher<visualization_msgs::msg::MarkerArray>(
    "global_map_query_kfs", rclcpp::QoS(10));
  pub_global_map_pts_opt_ =
    node_->create_publisher<sensor_msgs::msg::PointCloud2>("global_map_pts", rclcpp::QoS(10));
  pub_global_map_vis_ = node_->create_publisher<visualization_msgs::msg::MarkerArray>(
    "global_map_all_vis", rclcpp::QoS(10));
  pub_global_map_keypoints_vis_ = node_->create_publisher<visualization_msgs::msg::MarkerArray>(
    "global_map_keypoints_vis", rclcpp::QoS(10));
  pub_visible_fixed_landmarks_ =
    node_->create_publisher<sensor_msgs::msg::PointCloud2>("visible_fixed_landmarks", rclcpp::QoS(10));
  pub_global_map_matched_points_ =
    node_->create_publisher<sensor_msgs::msg::PointCloud2>("global_map_matched", rclcpp::QoS(10));
  pub_global_map_reobserved_points_ =
    node_->create_publisher<sensor_msgs::msg::PointCloud2>("global_map_reobserved", rclcpp::QoS(10));
  pub_global_map_reobserved_points_frontend_ = node_->create_publisher<sensor_msgs::msg::PointCloud2>(
    "global_map_reobserved_frontend", rclcpp::QoS(10));
  pub_global_map_point_ids_ = node_->create_publisher<visualization_msgs::msg::MarkerArray>(
    "global_map_point_ids", rclcpp::QoS(10));
#endif
}

void Visualizer::publishSvoInfo(
  const svo::FrameHandlerBase * const svo, const int64_t timestamp_nanoseconds)
{
  CHECK_NOTNULL(svo);
  ++trace_id_;
  ...
  asr_sdm_msgs::msg::Info msg_info;
  ...
  const int64_t timestamp_nanoseconds)
{
  ...
  auto msg_pose = std::make_unique<geometry_msgs::msg::PoseWithCovarianceStamped>();
  ...
  pub_imu_pose_->publish(std::move(msg_pose));
}

void Visualizer::publishCameraPoses(
  const FrameBundlePtr & frame_bundle, const int64_t timestamp_nanoseconds)
{
  const Eigen::Matrix4d T_cam_world_mat = frame_bundle->at(0)->T_cam_world().getTransformationMatrix();
  const Sophus::SE3d T_cam_world_sophus(T_cam_world_mat.block<3, 3>(0, 0), T_cam_world_mat.block<3, 1>(0, 3));
  vk::output_helper::publishTfTransform(
    T_cam_world_sophus, rclcpp::Time(timestamp_nanoseconds), "cam_pos", kWorldFrame, *br_);
  ...
  const FrameBundlePtr frames_ref, const FrameBundlePtr frames_cur, int64_t timestamp)
{
  ...
  for (size_t i = 0; i < frames_ref->size(); ++i) {
    ...
    publishFeatureTracks(
      frames_ref->at(i)->px_vec_, frames_cur->at(i)->px_vec_, matches_ref_cur,
  ...
  const Level & level, const uint64_t timestamp, const size_t frame_index)
{
  ...
  for (size_t i = 0; i < matches_ref_cur.size(); ++i) {
    ...
    cv::line(
      img_rgb, cv::Point2f(px_cur(0, i_cur) / scale, px_cur(1, i_cur) / scale),
      cv::Point2f(px_ref(0, i_ref) / scale, px_ref(1, i_ref) / scale), cv::Scalar(0, 255, 0), 2);
  }
  ...
  const FrameBundlePtr & frame_bundle, const int64_t timestamp, const bool draw_boundary)
{
  ...
  for (size_t i = 0; i < frame_bundle->size(); ++i) {
    ...
    if (draw_boundary) {
      cv::rectangle(
        img_rgb, cv::Point2f(0.0, 0.0), cv::Point2f(img_rgb.cols, img_rgb.rows),
        cv::Scalar(0, 255, 0), 6);
    }
    writeCaptionStr(img_rgb);
    cv_bridge::CvImage img_msg;
...
void Visualizer::visualizeHexacopter(const Transformation & T_frame_world, const uint64_t timestamp)
{
  if (pub_frames_->get_subscription_count() > 0) {
    vk::output_helper::publishCameraMarker(
      pub_frames_, "cam_pos", "cams", rclcpp::Time(timestamp), 1, 0.8,
      Vector3d(0., 0., 1.));
  }
}

void Visualizer::visualizeQuadrocopter(
  const Transformation & T_frame_world, const uint64_t timestamp)
{
  const Eigen::Matrix4d T_frame_world_mat = T_frame_world.getTransformationMatrix();
  const Sophus::SE3d T_frame_world_sophus(T_frame_world_mat.block<3, 3>(0, 0), T_frame_world_mat.block<3, 1>(0, 3));
  vk::output_helper::publishTfTransform(
    T_frame_world_sophus, rclcpp::Time(timestamp), "cam_pos", kWorldFrame, *br_);

  if (pub_frames_->get_subscription_count() > 0) {
    vk::output_helper::publishHexacopterMarker(
      pub_frames_, "cam_pos", "cams", rclcpp::Time(timestamp), 1, 0, 0.8,
      Vector3d(0., 0., 1.));
  }
}
...
void Visualizer::publishSeedsAsPointcloud(
  const Frame & frame, bool only_converged_seeds, bool reset_pc_before_publishing)
{
  ...
  pc_->header.stamp = node_->get_clock()->now().nanoseconds();
  pc_->reserve(frame.num_features_);
  for (size_t i = 0; i < frame.num_features_; ++i) {
    if (
      (only_converged_seeds && isConvergedCornerEdgeletSeed(frame.type_vec_.at(i))) ||
...
void Visualizer::publishSeedsUncertainty(const Map::Ptr & map)
{
  ...
  msg_variance.header.stamp = node_->get_clock()->now();
...
void Visualizer::publishMapRegion(const std::vector<FramePtr> & frames)
{
  ...
  if (pub_pc_->get_subscription_count() > 0) {
    ...
    pcl_conversions::toPCL(node_->get_clock()->now(), pc_->header.stamp);
    pc_->clear();
...
void Visualizer::publishKeyframeWithPoints(
  const FramePtr & frame, const uint64_t timestamp, const double marker_scale)
{
  // publish keyframe
  Transformation T_world_cam(frame->T_f_w_.inverse());
  vk::output_helper::publishFrameMarker(
    pub_frames_, T_world_cam.getRotationMatrix(), T_world_cam.getPosition(), "kfs",
    node_->get_clock()->now(), frame->id_ * 10, 0, marker_scale * 2.0);
  ...
  for (size_t i = 0; i < frame->num_features_; ++i) {
    ...
    if (isEdgelet(frame->type_vec_[i])) {
      vk::output_helper::publishPointMarker(
        pub_points_, xyz_world, "pts", node_->get_clock()->now(), id, 0, marker_scale * vis_scale_,
    ...
    /*
     ...
     "normal", node_->get_clock()->now(), point->id_, 0,  0.005,
     Vector3d(0.0, 0., 1.0));
     }
     */
  }
...
void Visualizer::publishActiveKeyframes(const std::vector<FramePtr> & active_kfs)
{
  ...
  for (size_t i = 0; i < active_kfs.size() - 1; i++) {
    vk::output_helper::publishLineMarker(
      pub_frames_, active_kfs[i]->pos(), active_kfs[i + 1]->pos(), ns, node_->get_clock()->now(), i,
      0,  // add the marker
  ...
  for (size_t cam_index = 0; cam_index < frame_bundle->size(); ++cam_index) {
    if (
      ...
      for (size_t i = 0; i < frame->num_features_; ++i) {
        ...
        asr_sdm_msgs::msg::Feature feature;
...
void Visualizer::visualizeCoordinateFrames(const Transformation & T_world_cam)
{
  ...
  // camera frame
  vk::output_helper::publishFrameMarker(
    pub_markers_, T_world_cam.getRotationMatrix(), T_world_cam.getPosition(), "cam",
    node_->get_clock()->now(), 0, 0, 0.2);

  // origin frame
  vk::output_helper::publishFrameMarker(
    pub_markers_, Matrix3d::Identity(), Vector3d::Zero(), kWorldFrame, node_->get_clock()->now(), 0, 0, 0.2);
}

#ifdef SVO_LOOP_CLOSING
void Visualizer::publishLoopClosureInfo(
  const LoopVizInfoVec & loop_viz_info, const std::string & ns, const Eigen::Vector3f & color,
  const double scale)
{
  publishLineList(node_, pub_loop_closure_, loop_viz_info, ns, color, scale);
}

bool Visualizer::publishPoseGraph(
  ...
  const size_t ignored_past_frames)
{
  ...
  if (pub_pose_graph_->get_subscription_count() > 0) {
    ...
    pcl_conversions::toPCL(node_->get_clock()->now(), pose_graph_pc.header.stamp);
    ...
    marker.action = visualization_msgs::msg::Marker::ADD;
    marker.scale.x = 0.08 * vis_scale_;
    marker.scale.y = 0.08 * vis_scale_;
    marker.scale.z = 0.08 * vis_scale_;
    marker.color.a = 1.0;
    marker.color.r = 0.0;
    marker.color.g = 0.0;
    marker.color.b = 1.0;

    marker.points.reserve(kf_list.size());
    for (size_t i = 0; i < kf_list.size(); i++) {
      const KeyFramePtr kf = kf_list[i];
      geometry_msgs::msg::Point p;
      p.x = kf->T_w_c_.getPosition()(0);
      p.y = kf->T_w_c_.getPosition()(1);
      p.z = kf->T_w_c_.getPosition()(2);
      marker.points.push_back(p);
    }
    ma.markers.emplace_back(std::move(marker));
    pub_pose_graph_->publish(ma);
  }
  ...
  if (pub_pose_graph_map_->get_subscription_count() > 0 && kf_list.size() > ignored_past_frames) {
    if (redo_pointcloud) {
      ...
      pose_graph_map_.clear();
      pcl_conversions::toPCL(node_->get_clock()->now(), pose_graph_map_.header.stamp);
...
void Visualizer::writeCaptionStr(cv::Mat img_rgb)
{
  if (viz_caption_str_) {
    cv::putText(
      img_rgb, img_caption_, cv::Point(20, 20), cv::FONT_HERSHEY_COMPLEX_SMALL, 1,
      cv::Scalar(0, 0, 250), 1, cv::LINE_AA);
  }
}

#ifdef SVO_GLOBAL_MAP
void Visualizer::visualizeGlobalMap(
  const GlobalMap & map, const std::string ns, const Eigen::Vector3f & color, const double scale)
{
  ...
  if (pub_global_map_keypoints_vis_->get_subscription_count() > 0) {
    ...
    publishLineList(
      pub_global_map_keypoints_vis_, vis, ns, Eigen::Vector3f(1.0, 1.0, 1.0), scale, 0.5);
  }
  ...
  if (pub_global_map_matched_points_.get_subscription_count() > 0) {
    ...
    publishLineList(
      pub_global_map_matched_points_, vis, ns, Eigen::Vector3f(1.0f, 0.0f, 0.0f), scale, 0.5);
  }
  ...
  if (pub_global_map_reobserved_points_.get_subscription_count() > 0) {
    ...
    publishLineList(
      pub_global_map_reobserved_points_, vis, ns, Eigen::Vector3f(0.0f, 1.0f, 0.0f), scale, 0.5);
  }
  ...
  if (pub_global_map_reobserved_points_frontend_.get_subscription_count() > 0) {
    ...
    publishLineList(
      pub_global_map_reobserved_points_frontend_, vis, ns, Eigen::Vector3f(0.0f, 1.0f, 1.0f), scale,
      0.5);
  }
  ...
  if (pub_global_map_pts_opt_.get_subscription_count() > 0) {
    ...
    publishPositionVecAsPC(
      pub_global_map_pts_opt_, {unstable_points, points, points_with_priors}, {30, 100, 200});
  }
...
void Visualizer::visualizeFixedLandmarks(const FramePtr & frame)
{
  ...
  publishLineList(
    pub_visible_fixed_landmarks_, vis, "visible_fixed_landmarks",
