...
#include <asr_sdm_vio/img_align/sparse_img_align_device_utils.cuh>  //!< for FloatTypeGpu
...
#include <asr_sdm_vio/common/seed_wrapper.h>
...
public:
  ...
  /// Default Constructor
  FrameGpu(
    const CameraPtr & cam, const cv::Mat & img, const uint64_t timestamp_ns,
    const size_t n_pyr_levels);

  /// Constructor without image. Just for testing!
  FrameGpu(
    const int id, const uint64_t timestamp_ns, const CameraPtr & cam,
