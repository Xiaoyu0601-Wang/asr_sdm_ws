...
Visualizer::Visualizer(rclcpp::Node::SharedPtr node) :
    ...
    publish_world_in_cam_frame_(vk::getParam<bool>(node_.get(), "publish_world_in_cam_frame", true)),
    publish_map_every_frame_(vk::getParam<bool>(node_.get(), "publish_map_every_frame", false)),
    publish_points_display_time_(rclcpp::Duration::from_seconds(vk::getParam<double>(node_.get(), "publish_point_display_time", 0.0))),
    T_world_from_vision_(Eigen::Matrix3d::Identity(), Eigen::Vector3d::Zero())
{
  // Init ROS2 Publishers
  pub_frames_ = node_->create_publisher<visualization_msgs::msg::Marker>("keyframes", 10);
  pub_points_ = node_->create_publisher<visualization_msgs::msg::Marker>("points", 1000);
  pub_pose_ = node_->create_publisher<geometry_msgs::msg::PoseWithCovarianceStamped>("pose", 10);
  pub_info_ = node_->create_publisher<svo_msgs::msg::Info>("info", 10);
  pub_dense_ = node_->create_publisher<svo_msgs::msg::DenseInput>("dense_input", 10);
    ...
    const double timestamp)
{
  ...
  if(pub_pose_->get_subscription_count() > 0 && slam.stage() == FrameHandlerBase::STAGE_DEFAULT_FRAME)
  {
    ...
    else
    {
      ...
      q = Eigen::Quaterniond(T_world_from_cam.rotationMatrix()*T_world_from_vision_.rotationMatrix().transpose());
...
void Visualizer::removeDeletedPts(const Map& map)
{
  if(pub_points_->get_subscription_count() > 0)
  {
    for(list<Point*>::const_iterator it=map.trash_points_.begin(); it!=map.trash_points_.end(); ++it)
      vk::output_helper::publishPointMarker(pub_points_, Eigen::Vector3d(), "pts", node_->now(), (*it)->id_, 2, 0.006, Eigen::Vector3d());
  }
}
...
void Visualizer::displayKeyframeWithMps(const FramePtr& frame, int ts)
{
  ...
  vk::output_helper::publishFrameMarker(
      ...
      T_world_cam.translation(), "kfs", node_->now(), frame->id_*10, 0, 0.015);
  ...
  for(Features::iterator it=frame->fts_.begin(); it!=frame->fts_.end(); ++it)
  {
    ...
    vk::output_helper::publishPointMarker(
        ...
        node_->now(), (*it)->point->id_, 0, 0.005, Eigen::Vector3d(1.0, 0., 1.0),
