...
#include "asr_sdm_vio/gtsam/camera_bearing_extrinsics_factor.h"
#include "asr_sdm_vio/gtsam/camera_bearing_factor.h"
...
GraphManager::GraphManager(const GraphManagerOptions & options) : options_(options)
{
}
...
void GraphManager::initialize(const double uplane_to_px_multiplier)
{
  ...
  double reproj_error_noise_uplane_ = options_.reproj_error_ns_px / uplane_to_px_multiplier;
  ...
  for (size_t level = 0u; level < kMaxPyrLevels; ++level) {
    ...
    if (options_.use_robust_px_noise) {
      ...
      uplane_noise_pyr_[level] = gtsam::noiseModel::Isotropic::Sigma(noise_dim, cur_noise);
    }
  }

  smart_noise_ = gtsam::noiseModel::Isotropic::Sigma(2, reproj_error_noise_uplane_);
  ...
  smart_factor_params_->setDynamicOutlierRejectionThreshold(smart_reproj_outlier_thresh_uplane);
  ...
  init_pose_prior_visual_inertial_.diagonal()[1] = init_pose_prior_visual_inertial_.diagonal()[0];
  ...
  init_pose_prior_visual_inertial_.diagonal()[4] = init_pose_prior_visual_inertial_.diagonal()[3];
  init_pose_prior_visual_inertial_.diagonal()[5] = init_pose_prior_visual_inertial_.diagonal()[3];

  // general priors
  default_point_prior_noise_ = gtsam::noiseModel::Diagonal::Sigmas(
    (gtsam::Vector(3) << gtsam::Vector3::Constant(options_.point_prior_sigma_meter)).finished());
  default_pose_prior_noise_ = gtsam::noiseModel::Diagonal::Sigmas(
    (gtsam::Vector(6) << gtsam::Vector3::Constant(options_.rotation_prior_sigma_rad),
  ...
  const ImuCalibration & imu_calib, const ImuInitialization & imu_init)
{
  ...
  imu_bias_prior_noise_ = gtsam::noiseModel::Diagonal::Sigmas(prior_bias_sigmas);

  // velocity prior
  velocity_prior_noise_ = gtsam::noiseModel::Isotropic::Sigma(3, imu_init.velocity_sigma);
  ...
  preintegration_params_ =
    PreintegratedImuMeasurements::Params::MakeSharedD(-imu_calib.gravity_magnitude);
...
void GraphManager::addExtrinsics(const svo::Transformation & T_b_c, const size_t cam_id)
{
  new_values_.insert(gtsam::Symbol('C', cam_id), gtsam::Pose3(T_b_c.getTransformationMatrix()));
}

void GraphManager::addObservationToLandmark(
  const svo::Frame & frame, const size_t keypoint_index, const bool refine_extrinsics)
{
  ...
  if (options_.use_bearing_factor) {
    ...
    if (refine_extrinsics) {
      new_factors_.push_back(
        boost::make_shared<CameraBearingTbcFactor>(
          f, uplane_noise_pyr_.at(level), gtsam::Symbol('x', pose_id),
          gtsam::Symbol('C', frame.nframe_index_), gtsam::Symbol('l', point_id)));
    } else {
      new_factors_.push_back(
        boost::make_shared<CameraBearingFactor3D>(
          f, uplane_noise_pyr_.at(level), gtsam::Symbol('x', pose_id), gtsam::Symbol('l', point_id),
    ...
    new_factors_.push_back(
      boost::make_shared<ProjectionFactor>(
        uv, uplane_noise_pyr_.at(level), gtsam::Symbol('x', pose_id), gtsam::Symbol('l', point_id),
        cam_calib_, gtsam::Pose3(frame.T_imu_cam().getTransformationMatrix())));
  }
...
// -----------------------------------------------------------------------------
void GraphManager::addSmartLandmark(const svo::Point & point, const Transformation & T_imu_cam)
{
  CHECK(
  ...
  SmartFactor::shared_ptr new_factor(
    new SmartFactor(smart_noise_, cam_calib_, T_body_cam, *smart_factor_params_));

  // book keeping
  point_id_to_new_smart_factor_map_.insert(std::make_pair(point.id(), new_factor));
...
void GraphManager::addObservationToSmartLandmark(
  const svo::Frame & frame, const size_t keypoint_index)
{
  const svo::Point & point = *CHECK_NOTNULL(frame.landmark_vec_.at(keypoint_index).get());

  // Update existing smart-factor.
  auto smart_factors_map_iter = point_id_to_smart_factor_info_map_.find(point.id());
  ...
  const Eigen::Vector2d uv = vk::project2(frame.f_vec_.col(static_cast<int>(keypoint_index)));
  ...
  if (smart_factors_map_iter->second.slot_in_graph_ != -1) {
    ...
    new_factor->add(uv, gtsam::Symbol('x', static_cast<size_t>(frame.bundleId())));
    point_id_to_new_smart_factor_map_.insert(std::make_pair(point.id(), new_factor));
...
void GraphManager::addCombinedImuFactor(
  const NFramesId & from_id, const NFramesId & to_id, const Eigen::Vector3d & acc_bias,
  const Eigen::Vector3d & gyr_bias, const double to_timestamp_sec,
  const ImuMeasurements & imu_measurements)
{
  ...
  new_factors_.push_back(
    boost::make_shared<gtsam::CombinedImuFactor>(
      gtsam::Symbol('x', from_id), gtsam::Symbol('v', from_id), gtsam::Symbol('x', to_id),
      gtsam::Symbol('v', to_id), gtsam::Symbol('b', from_id), gtsam::Symbol('b', to_id), pim));

  VLOG(40) << "Integrated " << imu_measurements.size() << " measurements"
           << " between frame " << from_id << " and " << to_id;
}

void GraphManager::addInitalPosePriorVI(const NFramesId & state_index, const Transformation & T_W_B)
{
...
void GraphManager::addPosePriorFactor(
  const NFramesId & state_index, const Transformation & T_W_B, const double rot_sig_rad,
...
void GraphManager::addPosePriorFactor(
  const NFramesId & state_index, const Transformation & T_W_B,
  const gtsam::SharedNoiseModel & noise)
{
  new_factors_.emplace_shared<gtsam::PriorFactor<gtsam::Pose3>>(
    gtsam::Symbol('x', state_index), gtsam::Pose3(T_W_B.getTransformationMatrix()), noise);
  registerNewFactor(
    state_index, new_factors_.size() - 1, &bundle_id_to_pose_prior_factor_id_map_,
    &bundle_id_to_prior_slot_id_map_);
}

void GraphManager::removePosePriorFactor(const NFramesId & state_index)
{
  deleteRegisteredFactor(
    state_index, &bundle_id_to_pose_prior_factor_id_map_, &bundle_id_to_prior_slot_id_map_);
}

void GraphManager::addPointPriorFactor(
  const PointId & point_id, const Eigen::Vector3d & pw, const double pos_sig)
{
  gtsam::SharedNoiseModel noise;
  if (pos_sig < 0) {
...
void GraphManager::addPointPriorFactor(
  const PointId & point_id, const Eigen::Vector3d & pw, const gtsam::SharedNoiseModel & noise)
{
  new_factors_.emplace_shared<gtsam::PriorFactor<gtsam::Point3>>(
    gtsam::Symbol('l', point_id), gtsam::Point3(pw), noise);
  registerNewFactor(
    point_id, new_factors_.size() - 1, &point_id_to_point_prior_factor_id_map_,
...
{
  deleteRegisteredFactor(
    point_id, &point_id_to_point_prior_factor_id_map_, &point_id_to_point_prior_slot_id_map_);
}

void GraphManager::addRangePriorFactor(
  const NFramesId & state_index, const PointId & point_id, const double dist,
  const Transformation & T_b_c, const gtsam::SharedNoiseModel & noise)
{
  CHECK_NOTNULL(noise.get());
  new_factors_.emplace_shared<CamPointDistFactor>(
    gtsam::Symbol('x', state_index), gtsam::Symbol('l', point_id), dist, noise,
    gtsam::Pose3(T_b_c.getTransformationMatrix()));
  registerNewFactor(
    point_id, new_factors_.size() - 1, &point_id_to_range_prior_factor_id_map_,
...
{
  deleteRegisteredFactor(
    point_id, &point_id_to_range_prior_factor_id_map_, &point_id_to_range_prior_slot_id_map_);
}

void GraphManager::addRelativePosePriorFactor(
  const NFramesId & bundle_id_1, const NFramesId & bundle_id_2, const Transformation & T_p1_p2,
  const gtsam::SharedNoiseModel & noise)
{
  CHECK_NOTNULL(noise.get());
  ...
  registerNewFactor(
    bundle_id_1, new_factors_.size() - 1, &first_bundle_id_to_relative_prior_factor_id_map_,
    &first_bundle_id_to_prior_slot_id_map_);
  CHECK(
    first_bundle_id_to_second_bundle_id_map_.find(bundle_id_1) ==
    first_bundle_id_to_second_bundle_id_map_.end());
  first_bundle_id_to_second_bundle_id_map_[bundle_id_1] = bundle_id_2;
}

void GraphManager::removeRelativePosePriorFactor(
  const NFramesId & bundle_id_1, const NFramesId & bundle_id_2)
{
  ...
  deleteRegisteredFactor(
    bundle_id_1, &first_bundle_id_to_relative_prior_factor_id_map_,
    &first_bundle_id_to_prior_slot_id_map_);
  first_bundle_id_to_second_bundle_id_map_.erase(sec_it);
}

void GraphManager::addBiasPriorFactor(
  const NFramesId & state_index, const Eigen::Vector3d & acc_bias, const Eigen::Vector3d & gyr_bias)
{
  new_factors_.push_back(
  ...
  VLOG(40) << "Priors - Bias Mean: Acc (" << acc_bias.transpose() << "), Gyro ( "
           << gyr_bias.transpose() << ")";
...
void GraphManager::addZeroVelocityPriorFactor(const NFramesId & state_index)
{
  new_factors_.push_back(
    boost::make_shared<gtsam::PriorFactor<gtsam::Vector3>>(
      gtsam::Symbol('v', state_index), Eigen::Vector3d::Zero(), zero_velocity_prior_noise_));
}

void GraphManager::augmentStateWithPose(const NFramesId & state_index, const Transformation & T_W_B)
{
...
void GraphManager::augmentStateWithVelocityAndBias(
  const NFramesId & state_index, const Eigen::Vector3d & W_v, const Eigen::Vector3d & acc_bias,
...
void GraphManager::addPointMatchFactor(const PointId first, const PointId second)
{
  new_factors_.push_back(
    boost::make_shared<PointMatchFactor>(
      gtsam::Symbol('l', first), gtsam::Symbol('l', second), gtsam::Point3(0.0, 0.0, 0.0),
      point_match_noise_));
}

void GraphManager::registerNewFactor(
  const int id, const size_t factor_idx, IdToFactorIdxMap * id_to_factor_idx_map,
...
void GraphManager::deleteRegisteredFactor(
  const int id, IdToFactorIdxMap * id_to_factor_idx_map, IdToSlotIdMap * id_to_slot_id_map)
{
...
void GraphManager::getUpdatesCopy(
  gtsam::NonlinearFactorGraph * new_factors, gtsam::Values * new_states,
  GTSAMSlotIndices * slots_to_delete, std::vector<int> * new_smart_factor_point_ids)
{
  ...
  for (auto & s : point_id_to_new_smart_factor_map_) {
    ...
    if (it->second.slot_in_graph_ != -1) {
      ...
      slots_to_delete->push_back(static_cast<size_t>(it->second.slot_in_graph_));
    }
  }
...
void GraphManager::updateSlotInfo(
  const gtsam::FastVector<size_t> & new_slots, const std::vector<int> & new_smart_factor_point_ids)
{
  // update slots of new inserted indices:
  VLOG(1) << ">>> === will update slots info ===:" << std::endl;
  VLOG(1) << "- # new_slots: " << new_slots.size() << "; "
          << "- # new smart_factor_pt_ids: " << new_smart_factor_point_ids.size() << "; "
          << "- # new pose prior: " << bundle_id_to_pose_prior_factor_id_map_.size() << "; "
          << "- # new point position prior: " << point_id_to_point_prior_factor_id_map_.size()
          << "; "
          << "- # new point range prior: " << point_id_to_range_prior_factor_id_map_.size() << "; "
          << "- # new relative pose prior: "
          << first_bundle_id_to_relative_prior_factor_id_map_.size() << std::endl;
  for (size_t i = 0; i < new_smart_factor_point_ids.size(); ++i) {
    const auto & it = point_id_to_smart_factor_info_map_.find(new_smart_factor_point_ids.at(i));
  ...
  updateSlotInfoSingle(
    new_slots, bundle_id_to_pose_prior_factor_id_map_, &bundle_id_to_prior_slot_id_map_);
  bundle_id_to_pose_prior_factor_id_map_.clear();

  updateSlotInfoSingle(
    new_slots, point_id_to_point_prior_factor_id_map_, &point_id_to_point_prior_slot_id_map_);
  point_id_to_point_prior_factor_id_map_.clear();

  updateSlotInfoSingle(
    new_slots, point_id_to_range_prior_factor_id_map_, &point_id_to_range_prior_slot_id_map_);
  point_id_to_range_prior_factor_id_map_.clear();

  updateSlotInfoSingle(
    new_slots, first_bundle_id_to_relative_prior_factor_id_map_,
    &first_bundle_id_to_prior_slot_id_map_);
  first_bundle_id_to_relative_prior_factor_id_map_.clear();
}
