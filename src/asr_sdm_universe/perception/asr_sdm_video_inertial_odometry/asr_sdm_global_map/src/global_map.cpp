...
#include "asr_sdm_vio/gtsam/graph_manager.h"
#include "asr_sdm_vio/gtsam/gtsam_optimizer.h"
#include "asr_sdm_vio/null_stream.h"
...
namespace
{
void calculateUnitBearingFromPoint(
  const svo::FramePtr & frame, const svo::PointPtr & pt, Eigen::Vector3d * f)
{
  (*f) = frame->pos() - pt->pos();
  f->normalize();
}
...
void CachedPointObservations::addObservation(const FramePtr & f, const size_t ftr_idx)
{
  ...
  if (!is_parallax_good_ && obs_unit_bearing_from_pt_.size() >= 2) {
    for (size_t i = 0; i < obs_unit_bearing_from_pt_.size() - 1; i++) {
      const double parallax =
        std::acos(obs_unit_bearing_from_pt_[i].dot(obs_unit_bearing_from_pt_.back()));
  ...
  fim_ += J.transpose() * J * (1.0 / std::pow(1.0 / f->getErrorMultiplier(), 2));
  Eigen::FullPivLU<Eigen::Matrix3d> lu_decomp(fim_);
  if (lu_decomp.rank() == 3) {
    Eigen::JacobiSVD<Eigen::MatrixXd> svd(fim_);
    cur_cond_num_ = svd.singularValues()(0) / svd.singularValues()(svd.singularValues().size() - 1);
...
GlobalMap::GlobalMap(
  const GlobalMapOptions & options, const GraphManagerOptions & graph_manager_options,
  const GTSAMOptimizerOptions & gtsam_opt_options, const CameraBundlePtr & ncams)
: options_(options), out_((options_.print_stdcout ? std::cout : kNullOutput)), ncams_(ncams)
{
  ...
  relative_pose_prior_ = gtsam::noiseModel::Diagonal::Sigmas(
    (gtsam::Vector(6) << gtsam::Vector3::Constant(options_.relative_rot_sigma_rad),
...
void GlobalMap::initializeIMUParams(
  const ImuCalibration & imu_calib, const ImuInitialization & imu_init)
{
...
void GlobalMap::addKeyframe(const FramePtr & new_frame)
{
  ...
  if (options_.ignore_redundant_landmarks && !reprojected_landmark_grid_) {
    ...
    reprojected_landmark_grid_.reset(new OccupandyGrid2D(
      cell_size, OccupandyGrid2D::getNCell(new_frame->cam()->imageWidth(), cell_size),
  ...
  // add to the id - keyframe map
  {
    ...
    if (options_.use_imu) {
      if (
        from_id_to_imu_meas_map_.find(new_frame->id()) == from_id_to_imu_meas_map_.end() &&
        imu_cur_start_kf_id_ != new_frame->id()) {
        out_ << "\n>>> Cannot find IMU measurements slot from frame id " << new_frame->id()
             << ", will not add to the graph." << std::endl;
        return;
      }
    }
    ...
    frame_id_to_unoptimized_pose_map_[new_frame->id()] = new_frame->T_world_imu();
  }
  const int cur_bundle_id = new_frame->bundleId();
  const int cur_frame_id = new_frame->id();
  skip_fixed_lm_from_frontend_ =
    (matched_new_frame_ids_.find(new_frame->id()) != matched_new_frame_ids_.end());
  ...
  bool same_bundle_already_added_ = false;
  {
    ...
    if (it != bundle_id_to_frame_id_map_.end()) {
      ...
      bundle_id_to_frame_id_map_[cur_bundle_id] = UniqueFrameIds{cur_frame_id};
    }
  }
  ...
  // update the graph
  {
    ...
    if (!same_bundle_already_added_) {
      ...
      if (options_.use_imu) {
        CHECK(
          frame_id_with_inertial_states_.find(new_frame->id()) ==
          frame_id_with_inertial_states_.end());
        graph_->augmentStateWithVelocityAndBias(
          new_frame->bundleId(), new_frame->imu_vel_w_, new_frame->imu_acc_bias_,
          new_frame->imu_gyr_bias_);
        ...
        if (prev_added_frame_id_ != -1) {
          ...
          getIMUMeasurementsBetween(
            prev_added_frame_id_, newly_added_frame_id_, true, &imu_meas_from_prev);

          CHECK(
            imu_constraints_frame_id_pairs_.find(prev_added_frame_id_) ==
            imu_constraints_frame_id_pairs_.end());
          const FramePtr & prev_f = frame_id_to_kf_in_graph_map_[prev_added_frame_id_];
          int prev_bundle_id = prev_f->bundleId();
          graph_->addCombinedImuFactor(
            prev_bundle_id, cur_bundle_id, prev_f->imu_acc_bias_, prev_f->imu_gyr_bias_,
            new_frame->getTimestampSec(), imu_meas_from_prev);
          imu_constraints_frame_id_pairs_[prev_added_frame_id_] = newly_added_frame_id_;
        }
      }
    } else {
    ...
    out_ << "Last added keyframe has " << frame_id_to_vis_pt_ids_in_graph_map_[cur_frame_id].size()
    ...
    if (frame_id_to_kf_in_graph_map_.size() >= options_.init_min_frame_before_opt) {
  ...
  if (initial_ba_fixed_) {
    out_ << ">>> === GLOBAL MAP INFO ===" << std::endl
         ...
         << "; "
         << "- " << pt_id_to_lm_in_graph_map_.size() << " landmarks" << std::endl
         << "- " << fixed_frame_ids_other_than_intial_.size()
         << " frames (excluding the initial one) and " << fixed_pts_all_.size()
         << " points with priors; "
         << "- " << fixed_relative_frame_id_pairs_.size() << " relative pose constraints."
         << std::endl
         << "- " << n_matched_points_in_graph_ << " matched points (diff ids) added in graph."
         << std::endl
         << "- " << n_reobserved_landmarks_in_graph_ << " reobservations (same ids) added in graph."
         << std::endl
         << "- " << n_reobserved_landmarks_in_graph_frontend_
         << " reobservations from frontend added in graph." << std::endl
         << "- " << this->numMatchesToAdd() << " point matches waited to be added from "
         << match_points_info_to_add_.size() << " frame matches." << std::endl
         << "- " << imu_constraints_frame_id_pairs_.size() << " IMU constraints"
         << "; "
         << " - " << from_id_to_imu_meas_map_.size() << " imu measurement segments saved."
         << std::endl;
    if (needToOptimize()) {
      this->optimize();
    }
...
void GlobalMap::optimize()
{
  try {
    ...
    for (const auto & v : frame_id_to_kf_in_graph_map_) {
      std::cerr << "x" << v.first << ": " << v.second->pos().x() << ", " << v.second->pos().y()
                << ", " << v.second->pos().z() << "; ";
    }
    std::cerr << std::endl;
    ...
    for (const auto & v : pt_id_to_lm_in_graph_map_) {
      std::cerr << "l" << v.first << ": " << v.second->pos().x() << ", " << v.second->pos().y()
                << ", " << v.second->pos().z() << "; ";
    }
    std::cerr << std::endl;
    ...
    for (const auto & v : pt_id_to_lm_in_graph_map_) {
      Eigen::Vector3d pt =
        estimate.at<gtsam::Point3>(gtsam::Symbol('l', static_cast<size_t>(v.first))).vector();
      std::cerr << "l" << v.first << ": " << pt.x() << ", " << pt.y() << ", " << pt.z() << "; ";
    }
    std::cerr << std::endl;
    ...
    for (const auto & v : frame_id_to_kf_in_graph_map_) {
      const gtsam::Pose3 pose =
        estimate.at<gtsam::Pose3>(gtsam::Symbol('x', static_cast<size_t>(v.second->bundleId())));
...
void GlobalMap::updateStates()
{
  if (optimizer_->latest_estimate_state_index_ > last_updated_bundle_id_) {
    ...
    updateStatesWithEstimate(
      optimizer_->estimate_, optimizer_->latest_estimate_state_index_.load());
...
void GlobalMap::getIMUMeasurementsBetween(
  const int start_f_id, const int end_f_id, const bool remove,
  ImuMeasurements * imu_meas_from_start)
{
  ...
  for (; imu_it != from_id_to_imu_meas_map_.end();) {
    ...
    if (imu_it->second.front().timestamp_ <= end_f->getTimestampSec()) {
      imu_meas_from_start->insert(
        imu_meas_from_start->begin(), imu_meas_i.begin(), imu_meas_i.end());
      if (remove) {
  ...
  out_ << std::setprecision(std::numeric_limits<double>::digits10 + 1) << "Found "
       << imu_meas_from_start->size() << " IMU measurements from " << start_f_id << "("
       << imu_meas_from_start->back().timestamp_ << ")"
       << " to " << end_f_id << "(" << imu_meas_from_start->front().timestamp_ << ")" << std::endl;
}
...
bool GlobalMap::checkAndFixInitialBA()
{
  if (options_.debug_fix_all_poses) {
    ...
    size_t idx = 0;
    for (auto it = frame_id_to_kf_in_graph_map_.cbegin(); it != frame_id_to_kf_in_graph_map_.cend();
         it++) {
  ...
  for (auto it = frame_id_to_kf_in_graph_map_.cbegin(); it != frame_id_to_kf_in_graph_map_.cend();
       it++) {
    const FramePtr & f = it->second;
    const UniquePointIds & vis_pts_in_ba = frame_id_to_vis_pt_ids_in_graph_map_[f->id()];
  ...
  // pose and/or points fixation
  {
    if (options_.use_imu) {
      ...
      out_ << "=> Fixing frame state (position, yaw, bias, velocity) for " << init_bundle_id << "("
           << init_f.id() << ")..." << std::endl;
      graph_->addInitalPosePriorVI(init_bundle_id, init_f.T_world_imu());
      graph_->addBiasPriorFactor(init_bundle_id, init_f.imu_acc_bias_, init_f.imu_gyr_bias_);
      ...
      const UniquePointIds & max_vis_pts_in_ba = frame_id_to_vis_pt_ids_in_graph_map_[max_frame_id];
      ...
      out_ << "Fixed " << n_fixed_landmarks << " points in initial BA." << std::endl;
    }
  }
  ...
  if (options_.use_relative_pose_fixation) {
    ...
    out_ << "Fixed " << n_fixed_relative << " relative pose pairs in initial BA." << std::endl;
  }
...
void GlobalMap::updatePriors()
{
  ...
  if (options_.frame_fixation_release_check && options_.use_relative_pose_fixation) {
...
bool GlobalMap::checkAndFixRelativePose(const int prev_id, const int cur_id)
{
  ...
  const UniquePointIds & prev_vis_pts_in_graph = frame_id_to_vis_pt_ids_in_graph_map_[prev_id];
  const UniquePointIds & cur_vis_pts_in_graph = frame_id_to_vis_pt_ids_in_graph_map_[cur_id];
  size_t n_common_lm = numCommonPoints(cur_vis_pts_in_graph, prev_vis_pts_in_graph);
  out_ << "There are " << n_common_lm << " common landmarks between " << prev_id << " and "
       << cur_id << std::endl;
  ...
  if (add_rel_constraint) {
    out_ << "Too few common landmarks,"
         ...
         << " - " << cur_f->bundleId() << "(" << cur_f->id() << ")" << std::endl;
...
void GlobalMap::addRangePriorUnsafe(const Point & pt, const Frame & f, const double ratio_dist)
{
  const double dist = (pt.pos() - f.T_world_cam().getPosition()).norm();
  gtsam::SharedNoiseModel range_noies = gtsam::noiseModel::Isotropic::Sigma(1, ratio_dist * dist);
  graph_->addRangePriorFactor(f.bundleId(), pt.id(), dist, f.T_imu_cam(), range_noies);
}

void GlobalMap::fixFramePoseUnsafe(
  const FramePtr & f, const bool bookkeeping, const gtsam::SharedNoiseModel & noise)
{
  if (noise.get()) {
  ...
  if (bookkeeping) {
    CHECK(
      fixed_frame_ids_other_than_intial_.find(f->id()) == fixed_frame_ids_other_than_intial_.end());
    fixed_frame_ids_other_than_intial_.insert(f->id());
  }
}

void GlobalMap::fixRelativePoseUnsafe(
  const FramePtr & f1, const FramePtr & f2, const gtsam::SharedNoiseModel & noise)
{
  ...
  graph_->addRelativePosePriorFactor(f1->bundleId(), f2->bundleId(), T_1_2, noise);
  CHECK(fixed_relative_frame_id_pairs_.find(f1->id()) == fixed_relative_frame_id_pairs_.end());
...
void GlobalMap::checkAndRemoveRelativePosePriors()
{
  for (auto it = fixed_relative_frame_id_pairs_.begin();
    ...
    size_t n_common_lm = numCommonPoints(
      frame_id_to_vis_pt_ids_in_graph_map_[id1], frame_id_to_vis_pt_ids_in_graph_map_[id2]);
    if (n_common_lm > options_.relative_release_min_common) {
      ...
      out_ << "Released the fixation for frame pair: " << id1 << " and " << id2 << std::endl;
    } else {
      it++;
    }
  }
...
void GlobalMap::addVisualMeasurementsToGraph(const FramePtr & new_frame)
{
  ...
  for (size_t ftr_idx = 0; ftr_idx < new_frame->numFeatures(); ftr_idx++) {
    ...
    if (it_in_ba != pt_id_to_lm_in_graph_map_.end()) {  // if it is already added to the graph
      CHECK(it_cached == pt_id_to_cached_pt_obs_map_.end());
      {
        this->addObservationToLandmark(new_frame, ftr_idx);
      }
    } else if (it_cached == pt_id_to_cached_pt_obs_map_.end()) {  // if it is not in the graph and
                                                                  // not cahced yet
  ...
  {
    for (auto it = pt_id_to_cached_pt_obs_map_.begin(); it != pt_id_to_cached_pt_obs_map_.cend();) {
      // sanity check
      CHECK_EQ(it->first, it->second.point_->id());
      CHECK(pt_id_to_lm_in_graph_map_.find(it->first) == pt_id_to_lm_in_graph_map_.end());
      ...
      if (it->second.isPointGood()) {
        ...
        if (options_.add_range_prior_to_good_pt && !options_.use_smart_factor) {
          addRangePriorUnsafe(*(it->second.point_), *(it->second.ob_frames_.front()), 0.8);
        }
        // bookkeeping
        it = pt_id_to_cached_pt_obs_map_.erase(it);
        n_added_good_landmarks++;
      } else if (options_.max_point_age > 0 && it->second.age_ >= options_.max_point_age) {
  ...
  out_ << "- Points cached but not added: " << pt_id_to_cached_pt_obs_map_.size() << std::endl;
  ...
  if (options_.verbose) {
    ...
    for (const auto id : good_lm_ids) {
      ...
      out_ << id << ": " << p.pos().x() << ", " << p.pos().y() << ", " << p.pos().z() << "; ";
    }
    out_ << std::endl;
  }
}
...
void GlobalMap::addReobservedLandmarkToGraph(const FramePtr & new_frame)
{
  ...
  for (size_t ftr_idx = 0; ftr_idx < new_frame->numFeatures(); ftr_idx++) {
    ...
    ftr_info[kNumPrevObs] = std::count_if(
      ...
      [&](const KeypointIdentifier & v) { return v.frame_id < new_frame->id(); });
    ftr_idx_to_n_observed.emplace_back(std::make_pair(ftr_idx, ftr_info));
  }

  if (ftr_idx_to_n_observed.size() > options_.max_reprojected_fixed_landmark_each_frame) {
    std::nth_element(
      ftr_idx_to_n_observed.begin(),
      ftr_idx_to_n_observed.begin() + options_.max_reprojected_fixed_landmark_each_frame,
      ftr_idx_to_n_observed.end(),
      [&](const std::pair<size_t, FtrInfo> & lhs, const std::pair<size_t, FtrInfo> & rhs) {
  ...
  out_ << "- Added " << n_added_fixed
       << " reobserved fixed landmarks "
          "from the frontend.\n";
}
...
void GlobalMap::updateFixedLandmarks()
{
  ...
  for (auto it = lm_id_to_num_reobs_map_.begin(); it != lm_id_to_num_reobs_map_.end();) {
    if (
      it->second >= static_cast<size_t>(options_.fix_lm_reobs_threshold) &&
      isPointOptimized(it->first)) {
      graph_->addPointPriorFactor(it->first, pt_id_to_lm_in_graph_map_[it->first]->pos(), -1);
...
void GlobalMap::addMatchingPointsToGraph()
{
  ...
  for (auto match_it = match_points_info_to_add_.begin();
    ...
    if (!frameInGraph(cur_match_info.lc_kf_id_) || !frameInGraph(cur_match_info.cur_kf_id_)) {
      match_it++;
      continue;
    }
    ...
    for (auto pit = cur_match_info.pt_id_matches_.begin();
      ...
      if (first_id == second_id) {
        ...
        for (const int f_id : {cur_match_info.lc_kf_id_, cur_match_info.cur_kf_id_}) {
          if (pointVisibleInFrameInGraph(first_id, f_id)) {
            continue;
          }
          bool res =
            addObservationToLandmarkByPointId(frame_id_to_kf_in_graph_map_[f_id], first_id);
        ...
        if (all_in_graph) {
          ...
          for (const int f_id : {cur_match_info.lc_kf_id_, cur_match_info.cur_kf_id_}) {
            registerFrameRelatedPoints(f_id, first_id, &frame_id_to_reobs_pts_ids_);
          }
        }
        ...
        registerFrameRelatedPoints(
          cur_match_info.lc_kf_id_, first_id, &frame_id_to_matched_pts_id_);
        registerFrameRelatedPoints(
          cur_match_info.cur_kf_id_, second_id, &frame_id_to_matched_pts_id_);
    ...
    // move the next one
    match_it =
      (cur_match_info.pt_id_matches_.empty() ? match_points_info_to_add_.erase(match_it)
                                             : std::next(match_it));
  }
...
bool GlobalMap::isFeatureGoodToAdd(const Frame & f, const size_t ftr_idx)
{
  ...
  if (point->n_succeeded_reproj_ < point->n_failed_reproj_ || point->n_failed_reproj_ > 10) {
    return false;
  }

  if (reprojected_landmark_grid_) {
    const size_t cur_cell_idx =
      reprojected_landmark_grid_->getCellIndex(f.px_vec_.col(static_cast<int>(ftr_idx)));
...
void GlobalMap::addCachedPointAndObservations(const CachedPointObservations & cached)
{
  ...
  CHECK(pt_id_to_lm_in_graph_map_.find(cached.point_->id()) == pt_id_to_lm_in_graph_map_.end());
...
void GlobalMap::addLandmark(svo::Point & pt, const Frame & f)
{
  ...
  CHECK(
    point_id_to_vis_frame_ids_in_graph_map_.find(pt.id()) ==
    point_id_to_vis_frame_ids_in_graph_map_.end());
  point_id_to_vis_frame_ids_in_graph_map_[pt.id()] = UniqueFrameIds{};
}

void GlobalMap::addObservationToLandmark(const FramePtr & f, const size_t ftr_idx)
{
  ...
  registerPointVisibleToFrame(f->id(), f->landmark_vec_[ftr_idx]->id(), ftr_idx);
}

bool GlobalMap::addObservationToLandmarkByPointId(const FramePtr & f, const int point_id)
{
  CHECK(f != nullptr);
  bool suc = false;
  for (size_t ftr_idx = 0; ftr_idx < f->numFeatures(); ftr_idx++) {
    if (f->landmark_vec_[ftr_idx] != nullptr && f->landmark_vec_[ftr_idx]->id() == point_id) {
  ...
  const gtsam::Values & estimate, const BundleId last_optimized_state_index)
{
  ...
  for (auto & v : frame_id_to_kf_in_graph_map_) {
    ...
    gtsam::Key pose_key = gtsam::Symbol('x', static_cast<size_t>(f->bundleId()));
    ...
    for (const auto pt_id : it_vis->second) {
      ...
      CHECK(f->landmark_vec_[ftr_idx])
        << "Feature type: " << str(f->type_vec_[ftr_idx]) << "; frame id: " << f->id();

      if (isPointOptimized(pt_id)) {
        f->type_vec_[ftr_idx] =
          isPointStable(pt_id) ? FeatureType::kFixedLandmark : FeatureType::kOutlier;
        continue;
      }
      ...
      if (getPointFromGraphByIdUnsafe(pt_id, estimate, &new_pos)) {
        if (
          options_.point_stable_thresh_meter > 0 &&
  ...
  const double stable_rate = (1.0 * stable_point_ids_.size() / updated_point_ids_.size());
  if (stable_rate > options_.inc_version_stable_ratio) {
    Point::global_map_value_version_++;
  }
  out_ << "Updated " << updated_point_ids_.size() << " with " << stable_point_ids_.size()
       << " points stable (" << stable_rate * 100 << ")." << std::endl;
}

void GlobalMap::getOverlapKeyframesMaxN(
  const Frame & query_f, const int max, std::vector<FramePtr> * overlap_kfs)
{
  ...
  for (auto & p : frame_ids_to_query_from_) {
    ...
    frame_id_to_dist_vec.emplace_back(std::make_pair(kf->id(), (kf->pos() - cam_pos).norm()));
  }
  std::sort(
  ...
  size_t max_kfs = max > 0 ? static_cast<size_t>(max) : frame_id_to_dist_vec.size();
  size_t n_found = 0;
...
void GlobalMap::getAllKFPositions(
  rpg::PositionVec * kf_pos, rpg::PositionVec * kf_pos_with_priors) const
{
  ...
  for (const auto & v : frame_id_to_kf_in_graph_map_) {
    ...
    if (
      fixed_relative_frame_id_pairs_.find(v.first) != fixed_relative_frame_id_pairs_.end() ||
...
void GlobalMap::getQueryKFPositions(
  rpg::PositionVec * query_kf_pos, rpg::PositionVec * last_queried_kf_pos) const
{
...
void GlobalMap::getBundleIDIMUPosesMap(BundleIdToIMUPose * pose_map) const
{
  ...
  const gtsam::Values & estimate = optimizer_->estimate_;
  for (const auto & v : frame_id_to_kf_in_graph_map_) {
    gtsam::Key pose_key = gtsam::Symbol('x', static_cast<size_t>(v.second->bundleId()));
...
void GlobalMap::getAllPoints(
  rpg::PositionVec * points, rpg::PositionVec * points_with_priors,
...
bool GlobalMap::getPointFromGraphByIdUnsafe(
  const int pt_id, const gtsam::Values & estimate, Eigen::Vector3d * pt) const
{
  CHECK_NOTNULL(pt);
...
void GlobalMap::getVizLinks(VisibilityInfoVec * vis_info, const int last_n) const
{
  ...
  int cnt = 0;
  for (auto rit = frame_id_to_kf_in_graph_map_.rbegin(); rit != frame_id_to_kf_in_graph_map_.rend();
       rit++) {
...
void GlobalMap::getKeypointsVisLinks(VisibilityInfoVec * vis_info) const
{
  ...
  for (const auto & e : frame_id_to_kf_in_graph_map_) {
    ...
    for (const auto & pt : f->key_pts_) {
      ...
      vis_info->emplace_back(
        (VisibilityInfo() << f_pos.x(), f_pos.y(), f_pos.z(), pt_pos.x(), pt_pos.y(), pt_pos.z())
          .finished());
    }
  }
}

void GlobalMap::getPointPositionAndIds(rpg::PositionVec * points, std::vector<int> * ids) const
{
  CHECK_NOTNULL(points);
  points->clear();
  CHECK_NOTNULL(ids);
...
void GlobalMap::framePointsToVisLink(
  const std::map<int, UniquePointIds> & d, VisibilityInfoVec * vis_info) const
{
...
bool GlobalMap::traceStampedPoses(const std::string & trace_fn)
{
  ...
  if (!trace) {
    ...
    for (const auto & p : frame_id_to_kf_in_graph_map_) {
      ...
      trace << p.second->getTimestampSec() << " " << pose_imu.getPosition()(0, 0) << " "
