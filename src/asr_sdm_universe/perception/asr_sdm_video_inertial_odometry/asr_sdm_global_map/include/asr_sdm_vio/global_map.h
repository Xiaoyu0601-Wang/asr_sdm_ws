...
struct CachedPointObservations
{
  ...
  inline bool isPointGood() const
  {
    return hasEnoughObs() && is_parallax_good_ && point_->nRefs() == 0 &&
           (cond_thresh < 0 || cur_cond_num_ < cond_thresh) && cur_mean_det_ > fim_det_cbr_minimum;
  }
};
...
public:
  ...
  void initializeIMUParams(const ImuCalibration & imu_calib, const ImuInitialization & imu_init);

  inline size_t numFramesInMap() const { return frame_id_to_kf_in_graph_map_.size(); }
  ...
  void getAllKFPositions(rpg::PositionVec * kf_pos, rpg::PositionVec * kf_pos_with_priors) const;
  void getQueryKFPositions(
  ...
  void getPointPositionAndIds(rpg::PositionVec * points, std::vector<int> * ids) const;
  ...
  inline void getReobservedPointsVisLinksFrontend(VisibilityInfoVec * vis_info) const
  {
  ...
  inline void accumulateIMUMeasurements(const ImuMeasurements & imu_meas)
  {
    ...
    imu_meas_since_cur_start_kf_.insert(
      imu_meas_since_cur_start_kf_.begin(), imu_meas.begin(), imu_meas.end());
  }
  // Start new accumulation: called when a new keyframe is added
  inline void startNewAccumulation(const int new_start_kf_id)
  {
    if (imu_cur_start_kf_id_ != -1) {
      from_id_to_imu_meas_map_[imu_cur_start_kf_id_] = imu_meas_since_cur_start_kf_;
    }
  ...
  inline void addMatchingPointInfo(const MatchedPointsInfo & match_info)
  {
    ...
    if (match_points_info_to_add_.size() > kMaxN) {
      match_points_info_to_add_.erase(
        match_points_info_to_add_.begin() + kMaxN, match_points_info_to_add_.end());
    }
  }
  ...
  {
    return pt_id_to_lm_in_graph_map_.find(id) != pt_id_to_lm_in_graph_map_.end();
  }

  inline bool frameInGraph(const int frame_id)
  {
    return frame_id_to_kf_in_graph_map_.find(frame_id) != frame_id_to_kf_in_graph_map_.end();
  }

  inline bool hasNewEstimate() const { return last_published_bundle_id_ < last_updated_bundle_id_; }
  ...
  {
    if (
      options_.optimize_every_n < 0 ||
      n_since_last_optimization_ >= static_cast<size_t>(options_.optimize_every_n) ||
...
private:
  ...
  {
    return cam_id_to_Tbc_in_graph_map_.find(cam_id) != cam_id_to_Tbc_in_graph_map_.end();
  }
  ...
  // - Range priors
  void addRangePriorUnsafe(const Point & pt, const Frame & f, const double ratio_dist);

  // Bookkeeping functions
  // - visible points to frame
  inline void registerPointVisibleToFrame(const int frame_id, const int pt_id, const size_t ftr_idx)
  {
  ...
  {
    if (fixed_landmarks_in_graph_.find(pt_id) != fixed_landmarks_in_graph_.end()) {
      return;
    }
  ...
  inline void registerFrameRelatedPoints(
    const int frame_id, const int pt_id, std::map<int, UniquePointIds> * d)
  {
  ...
  // utility functions
  void framePointsToVisLink(const std::map<int, UniquePointIds> & d, VisibilityInfoVec * vis) const;
  inline void getKfToPointLink(const FramePtr & f, const int pt_id, VisibilityInfo * v) const
  {
    ...
    std::map<int, PointPtr>::const_iterator pt_cit = pt_id_to_lm_in_graph_map_.find(pt_id);
  ...
  inline void checkFrameFromGlobalMap(const FramePtr & f)
  {
    for (size_t ftr_idx = 0; ftr_idx < f->numFeatures(); ftr_idx++) {
      ...
      CHECK(isFixedLandmark(type) || type == FeatureType::kOutlier) << str(type) << std::endl;
  ...
  inline size_t numCommonPoints(const UniquePointIds & set1, const UniquePointIds & set2)
  {
    size_t n_common = 0;
