...
public:
  ...
  // smart factors
  void addSmartLandmark(const svo::Point & point, const Transformation & T_imu_cam);

  void addObservationToSmartLandmark(const svo::Frame & frame, const size_t keypoint_index);
  ...
  // Pose priors
  void addInitalPosePriorVI(const NFramesId & state_index, const Transformation & T_W_B);
  ...
  // Point priors
  void addPointPriorFactor(
    const PointId & state_index, const Eigen::Vector3d & pw, const double pos_sig);

  void addPointPriorFactor(
    const PointId & state_index, const Eigen::Vector3d & pw,
    const gtsam::SharedNoiseModel & noise_model);

  void removePointPriorFactor(const PointId & point_id);

  // range priors
  void addRangePriorFactor(
  ...
  void addRelativePosePriorFactor(
    const NFramesId & bundle_id_1, const NFramesId & bundle_id_2, const Transformation & T_p1_p2,
    const gtsam::SharedNoiseModel & noise);
  void removeRelativePosePriorFactor(const NFramesId & bundle_id_1, const NFramesId & bundle_id_2);

  // Inertial related
  void addBiasPriorFactor(
    const NFramesId & state_index, const Eigen::Vector3d & acc_bias,
    const Eigen::Vector3d & gyro_bias);

  void addVelocityPriorFactor(const NFramesId & state_index, const Eigen::Vector3d & W_v);

  void addZeroVelocityPriorFactor(const NFramesId & state_index);

  // Augment State
  void augmentStateWithPose(const NFramesId & state_index, const Transformation & T_W_B);
  ...
  void registerNewFactor(
    const int id, const size_t factor_idx, IdToFactorIdxMap * id_to_factor_idx_map,
    IdToSlotIdMap * id_to_slot_id_map);

  void deleteRegisteredFactor(
    const int id, IdToFactorIdxMap * id_to_factor_idx_map, IdToSlotIdMap *);

  // Retrieval (Always lock mut_ when using)
  void getUpdatesCopy(
    gtsam::NonlinearFactorGraph * graph_updates, gtsam::Values * value_updates,
    GTSAMSlotIndices * delete_indices, std::vector<int> * smart_factor_point_ids);

  // Note: this function relies on the fact that the smart factors are passed
  // first
  void updateSlotInfo(
    const gtsam::FastVector<size_t> & new_slots, const std::vector<int> & smart_factor_point_ids);
  void updateSlotInfoSingle(
    const gtsam::FastVector<size_t> & new_slots, const IdToFactorIdxMap & id_to_factor_idx_map,
    IdToSlotIdMap * id_to_slot_id_map);
  ...
  {
    return point_id_to_smart_factor_info_map_.find(pid) != point_id_to_smart_factor_info_map_.end();
  }
