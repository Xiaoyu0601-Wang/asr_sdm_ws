...
extern const std::unordered_map<svo::Stage, std::string, EnumClassHash> kStageName;

enum class TrackingQuality { kInsufficient, kBad, kGood };
extern const std::unordered_map<svo::TrackingQuality, std::string, EnumClassHash>
  kTrackingQualityName;

enum class UpdateResult { kDefault, kKeyframe, kFailure };
extern const std::unordered_map<svo::UpdateResult, std::string, EnumClassHash> kUpdateResultName;
...
public:
  ...
  bool addImageBundle(const std::vector<cv::Mat> & imgs, const uint64_t timestamp);
  ...
  inline bool isBackendScaleInitialised() const { return backend_scale_initialized_; }
  ...
  inline bool doesGlobalMapHaveInitialBA() const { return global_map_has_initial_ba_; }
  ...
  inline std::string UpdateResultStr() const { return kUpdateResultName.at(update_res_); }
...
protected:
  ...
  void optimizeStructure(const FrameBundlePtr & frames, int max_n_pts, int max_iter);
