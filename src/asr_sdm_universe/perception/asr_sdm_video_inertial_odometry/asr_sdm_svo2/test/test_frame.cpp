...
struct BearingVectorFunctor : public aslam::common::NumDiffFunctor<3, 6>
{
  ...
  virtual bool functional(
    const typename NumDiffFunctor::InputType & x, typename NumDiffFunctor::ValueType & fvec,
...
{
  UnitPlaneFunctor(
    const Transformation & T_cam_imu, const Eigen::Vector3d & p_in_imu,
...
{
  ImagePlaneFunctor(
    std::shared_ptr<const svo::Camera> cam, const Transformation & T_cam_imu,
...
TEST_F(SVOFrameTest, testUnitPlaneJacobian)
{
  ...
  for (int i = 0; i < 10; i++) {
    ...
    TEST_JACOBIAN_FINITE_DIFFERENCE(
      UnitPlaneFunctor, input, 1e-7, 1e-3, T_cam_imu, p_in_imu, f_in_imu);
  }
}
...
TEST_F(SVOFrameTest, testOmniImageJacobian)
{
  ...
  for (int i = 0; i < 10; i++) {
    ...
    TEST_JACOBIAN_FINITE_DIFFERENCE(
      ImagePlaneFunctor, input, 1e-4, 5 * 1e-2, omni_cam, T_cam_imu, p_in_imu);
  }
}
...
TEST_F(SVOFrameTest, testPinholeImageJacobian)
{
  ...
  for (int i = 0; i < 10; i++) {
    ...
    // on image plane, should be 2-order higher than unit plane error
    TEST_JACOBIAN_FINITE_DIFFERENCE(
      ImagePlaneFunctor, input, 1e-4, 5 * 1e-2, pinhole_cam, T_cam_imu, p_in_imu);
  }
