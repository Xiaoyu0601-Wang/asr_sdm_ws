    ...
    const bool remove_measurements)
{
  ...
  // Find the first measurement older than frame_timestamp,
  ...
  if(extracted_measurements.size() < 2u)
  {
    ...
    ImuMeasurements& measurements)
{
  ...
  // Find the right measurements for integration.
  // NOTE: newest measurement is at the front of the list!
  auto it1 = measurements_.end(); // older timestamp (<= t1)
  auto it2 = measurements_.end(); // newer timestamp (>= t2)
  bool it2_set = false;
