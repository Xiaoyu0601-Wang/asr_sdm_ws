...
FrameHandlerBase::FrameHandlerBase(
  const BaseOptions & base_options, const ReprojectorOptions & reprojector_options,
  const DepthFilterOptions & depthfilter_options, const DetectorOptions & detector_options,
...
bool FrameHandlerBase::addFrameBundle(const FrameBundlePtr & frame_bundle)
{
  ...
  if (bundle_adjustment_) {
    ...
    if (backend_reinit_) {
      bundle_adjustment_->setReinitStartValues(
        speed_bias_backend_latest_, T_WS_backend_latest_, timestamp_backend_latest_);
    }
  ...
  if (stage_ == Stage::kTracking) {
    if (isInRecovery()) {
      ...
      if (
        (new_frames_->getMinTimestampSeconds() - last_good_tracking_time_sec_) <
  ...
  if (update_res_ == UpdateResult::kFailure) {
    ...
    CHECK(
      stage_ == Stage::kTracking || stage_ == Stage::kInitializing ||
...
size_t FrameHandlerBase::sparseImageAlignment()
{
  ...
  if (have_motion_prior_) {
    ...
    sparse_img_align_->setWeightedPrior(
      T_newimu_lastimu_prior_, 0.0, 0.0, options_.img_align_prior_lambda_rot, prior_trans, 0.0,
      0.0);
  }
  ...
  for (size_t camera_idx = 0; camera_idx < cams_->numCameras(); ++camera_idx) {
    ...
    if (!isInRecovery() && cur_reprojector->options_.use_kfs_from_global_map && global_map_) {
      ...
      global_map_->getOverlapKeyframesMaxN(
        *new_frames_->at(camera_idx), cur_reprojector->options_.max_n_global_kfs,
        &overlap_kfs_global);
      overlap_kfs_.at(camera_idx)
        .insert(
          overlap_kfs_.at(camera_idx).end(), std::make_move_iterator(overlap_kfs_global.begin()),
...
size_t FrameHandlerBase::optimizePose()
{
  ...
  if (have_motion_prior_) {
    ...
    pose_optimizer_->setRotationPrior(
      new_frames_->get_T_W_B().getRotation().inverse(), options_.poseoptim_prior_lambda);
  }
  ...
  SVO_DEBUG_STREAM(
    "PoseOptimizer:" << "\t ErrInit = " << pose_optimizer_->stats_.reproj_error_before
...
void FrameHandlerBase::upgradeSeedsToFeatures(const FramePtr & frame)
{
  ...
  for (size_t i = 0; i < frame->num_features_; ++i) {
    if (frame->landmark_vec_[i]) {
      ...
      if (
        type == FeatureType::kCorner || type == FeatureType::kEdgelet ||
...
bool FrameHandlerBase::needNewKf(const Transformation &)
{
  ...
  if (options_.kfselect_criterion == KeyframeCriterion::DOWNLOOKING) {
    for (const auto & frame : visible_kfs) {
      ...
      if (
        fabs(relpos.x()) / depth_median_ < options_.kfselect_min_dist &&
  ...
  if (isBackendValid()) {
    if (
      last_kf_time_sec_ > 0 && options_.kfselect_backend_max_time_sec > 0 &&
  ...
  if (
    last_frames_->at(0)->id() - map_->last_added_kf_id_ <
  ...
  for (const auto & kf : visible_kfs) {
    ...
    const double a =
      Quaternion::log(new_frames_->at(0)->T_f_w_.getRotation() * kf->T_f_w_.getRotation().inverse())
        .norm() *
      180 / M_PI;
...
void FrameHandlerBase::getMotionPrior(const bool /*use_velocity_in_frame*/)
{
   ...
   */
    ...
    options_.poseoptim_prior_lambda > 0 || options_.img_align_prior_lambda_rot > 0 ||
