...
double PoseOptimizer::evaluateError(
  const Transformation & T_imu_world, HessianMatrix * H, GradientVector * g)
{
      ...
      if (isEdgelet(frame->type_vec_[i])) {
        ...
        if (err_type_ == ErrorType::kUnitPlane)
          ...
          pose_optimizer_utils::calculateEdgeletResidualImagePlane(
            ...
            *frame->cam(), measurement_sigma, robust_weight_, &unwhitened_error, &chi2_error, H, g);
...
void PoseOptimizer::removeOutliers(
  const double reproj_err_threshold, Frame * frame, std::vector<double> * reproj_errors,
  std::size_t * n_deleted_edges, std::size_t * n_deleted_corners)
{
  CHECK_NOTNULL(frame);
...
void PoseOptimizer::update(
  const State & T_imuold_world, const UpdateVector & dx, State & T_imunew_world)
{
