...
AbstractInitialization::AbstractInitialization(
  const InitializationOptions & init_options, const FeatureTrackerOptions & tracker_options,
...
InitResult HomographyInit::addFrameBundle(const FrameBundlePtr & frames_cur)
{
  ...
  // Compute model
  const vk::HomographyResult H_cur_ref = vk::estimateHomography(
    f_cur, f_ref, frames_ref_->at(0)->getErrorMultiplier(), options_.reproj_error_thresh,
    options_.init_min_inliers);
  if (H_cur_ref.score < options_.init_min_inliers) {
    SVO_WARN_STREAM(
      "Init Homography: Have " << H_cur_ref.score << "inliers. " << options_.init_min_inliers
...
InitResult FivePointInit::addFrameBundle(const FrameBundlePtr & frames_cur)
{
  ...
  if (matches_cur_ref.size() < 5u) {
    VLOG(3) << "5Pt Init: Not enough matches for 5pt. Have " << matches_cur_ref.size();
    return InitResult::kNoKeyframe;
  }

  // Guard against invalid bearings (NaN/Inf/zero-norm). These can cause OpenGV/Eigen
  // to segfault during SVD.
  auto isFiniteBearing = [](const Eigen::Vector3d& v) {
    return std::isfinite(v.x()) && std::isfinite(v.y()) && std::isfinite(v.z()) &&
           v.squaredNorm() > 1e-12;
  };

  for (size_t i = 0; i < f_cur.size(); ++i) {
    if (!isFiniteBearing(f_cur[i]) || !isFiniteBearing(f_ref[i])) {
      SVO_WARN_STREAM("5Pt Init: Invalid bearing vector detected. Skip initialization.");
      return InitResult::kNoKeyframe;
    }
  }
  ...
  try {
    ...
    SVO_WARN_STREAM("5Pt Init: OpenGV exception in computeModel(): " << e.what());
    return InitResult::kFailure;
  } catch (...) {
    SVO_WARN_STREAM("5Pt Init: Unknown exception in OpenGV computeModel().");
    return InitResult::kFailure;
  }
...
InitResult ArrayInitOptimization::addFrameBundle(const FrameBundlePtr & /*frames_cur*/)
{
  ...
  gtsam::Pose3 pose_ref(
    gtsam::Rot3(T_world_refimu.getRotationMatrix()), gtsam::Point3(T_world_refimu.getPosition()));
  graph.push_back(
...
bool triangulateAndInitializePoints(
  ...
  const double reprojection_threshold, const double depth_at_current_frame,
  const size_t min_inliers_threshold, AbstractInitialization::FeatureMatches & matches_cur_ref)
{
...
void triangulatePoints(
  const Frame & frame_cur, const Frame & frame_ref, const Transformation & T_cur_ref,
  const double reprojection_threshold, AbstractInitialization::FeatureMatches & matches_cur_ref,
  Positions & points_in_cur)
{
  ...
  for (size_t i = 0; i < matches_cur_ref.size(); ++i) {
    ...
    const Position xyz_in_cur = vk::triangulateFeatureNonLin(
    ...
    if (
      e1 > angle_threshold || e2 > angle_threshold ||
...
void rescaleAndInitializePoints(
  ...
  const AbstractInitialization::FeatureMatches & matches_cur_ref, const Positions & points_in_cur,
  const Transformation & T_cur_ref, const double depth_at_current_frame)
{
...
AbstractInitialization::UniquePtr makeInitializer(
  const InitializationOptions & init_options, const FeatureTrackerOptions & tracker_options,
  const DetectorOptions & detector_options, const CameraBundle::Ptr & cams)
{
...
void copyBearingVectors(
  const Frame & frame_cur, const Frame & frame_ref,
  const AbstractInitialization::FeatureMatches & matches_cur_ref,
  AbstractInitialization::BearingVectors * f_cur, AbstractInitialization::BearingVectors * f_ref)
{
  CHECK_NOTNULL(f_cur);
  CHECK_NOTNULL(f_ref);
