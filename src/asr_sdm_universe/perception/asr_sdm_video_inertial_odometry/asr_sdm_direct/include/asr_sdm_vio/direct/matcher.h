...
public:
  ...
  struct Options
  {
    ...
    int align_max_iter =
      10;  //!< number of iterations for aligning the feature patches in gauss newton
    ...
    bool subpix_refinement =
      true;  //!< do gauss newton feature patch alignment after epipolar search
  ...
  double epi_length_pyramid_;  //!< length of epipolar line segment in pixels on pyrimid level (only
                               //!< used for epipolar search)
...
void createPatchWithBorderNoWarp(
  const cv::Mat & img, const Eigen::Vector2i & px, const int halfpatch_size_without_border,
  uint8_t * patch_with_border);

/// returns a patch that is 2px smaller in both dimensions.
void createPatchFromPatchWithBorder(
  const uint8_t * const patch_with_border, const int patch_size, uint8_t * patch);
