  ...
  const FloatType & ref_depth, Keypoint & px_cur)
{
  ...
  */
  if (options_.use_affine_warp_) {
    if (!warp::warpAffine(
          A_cur_ref_, ref_frame.img_pyr_[ref_ftr.level], ref_ftr.px, ref_ftr.level, search_level_,
    ...
    if (!warp::warpPixelwise(
          cur_frame, ref_frame, ref_ftr, ref_ftr.level, search_level_, kHalfPatchSize + 1,
          patch_with_border_))
      return MatchResult::kFailWarp;
  }
  ...
  if (isEdgelet(ref_ftr.type)) {
    ...
    if (feature_alignment::align1D(
          cur_frame.img_pyr_[search_level_], dir_cur, patch_with_border_, patch_,
  ...
  const double d_estimate_inv, const double d_min_inv, const double d_max_inv, double & depth)
{
  ...
  return findEpipolarMatchDirect(
    ref_frame, cur_frame, T_cur_ref, ref_ftr, d_estimate_inv, d_min_inv, d_max_inv, depth);
}
  ...
  const double d_max_inv, double & depth)
{
  ...
  warp::getWarpMatrixAffine(
    ref_frame.cam_, cur_frame.cam_, ref_ftr.px, ref_ftr.f, 1.0 / std::max(0.000001, d_estimate_inv),
...
Matcher::MatchResult Matcher::findLocalMatch(
  const Frame & frame, const Eigen::Ref<GradientVector> & direction, const int patch_level,
  Keypoint & px_cur)
{
  ...
  if (options_.align_1d)
    res = feature_alignment::align1D(
      frame.img_pyr_[patch_level], direction, patch_with_border_, patch_, options_.align_max_iter,
      options_.affine_est_offset_, options_.affine_est_gain_, &px_scaled, &h_inv_);
  else
    res = feature_alignment::align2D(
      frame.img_pyr_[patch_level], patch_with_border_, patch_, options_.align_max_iter,
...
bool Matcher::updateZMSSD(
  const Frame & frame, const Eigen::Vector2i & pxi, const int patch_level,
...
Matcher::MatchResult depthFromTriangulation(
  const Transformation & T_search_ref, const Eigen::Vector3d & f_ref, const Eigen::Vector3d & f_cur,
  double * depth)
{
