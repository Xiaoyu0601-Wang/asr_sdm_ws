...
: options_(options),
  grid_(
    options_.cell_size, std::ceil(static_cast<double>(cam->imageWidth()) / options_.cell_size),
  ...
  closeness_check_grid_(
    ...
    std::ceil(
      options_.sec_grid_fineness * static_cast<double>(cam->imageWidth()) / options_.cell_size),
    std::ceil(
      options_.sec_grid_fineness * static_cast<double>(cam->imageHeight()) / options_.cell_size))
{
}
...
void AbstractDetector::detect(const FramePtr & frame)
{
  ...
  detect(
    frame->img_pyr_, frame->getMask(), max_n_features, frame->px_vec_, frame->score_vec_,
...
void FastDetector::detect(
  const ImgPyr & img_pyr, const cv::Mat & mask, const size_t max_n_features, Keypoints & px_vec,
  Scores & score_vec, Levels & level_vec, Gradients & grad_vec, FeatureTypes & types_vec)
{
  Corners corners(grid_.n_cols * grid_.n_rows, Corner(0, 0, options_.threshold_primary, 0, 0.0f));
  fd_utils::fastDetector(
    img_pyr, options_.threshold_primary, options_.border, options_.min_level, options_.max_level,
    corners, grid_);
  fd_utils::fillFeatures(
    corners, FeatureType::kCorner, mask, options_.threshold_primary, max_n_features, px_vec,
  ...
  Scores & score_vec, Levels & level_vec, Gradients & grad_vec, FeatureTypes & types_vec)
{
  ...
  fd_utils::fillFeatures(
    corners, FeatureType::kEdgelet, mask, options_.threshold_secondary, max_n_features, px_vec,
    score_vec, level_vec, grad_vec, types_vec, grid_);
}
  ...
  Scores & score_vec, Levels & level_vec, Gradients & grad_vec, FeatureTypes & types_vec)
{
  ...
  fd_utils::edgeletDetector_V2(
    img_pyr, options_.threshold_secondary, options_.border, options_.min_level, options_.max_level,
    corners, grid_);
  fd_utils::fillFeatures(
    corners, FeatureType::kEdgelet, mask, options_.threshold_secondary, max_n_features, px_vec,
    score_vec, level_vec, grad_vec, types_vec, grid_);
  ...
  Scores & score_vec, Levels & level_vec, Gradients & grad_vec, FeatureTypes & types_vec)
{
  ...
  if (max_features > 0) {
    // Detect edgelets.
    Corners corners(
      grid_.n_cols * grid_.n_rows, Corner(0, 0, options_.threshold_secondary, 0, 0.0f));
    fd_utils::edgeletDetector_V2(
      img_pyr, options_.threshold_secondary, options_.border, options_.min_level,
      options_.max_level, corners, grid_);
    fd_utils::fillFeatures(
      corners, FeatureType::kEdgelet, mask, options_.threshold_secondary, max_features, px_vec,
      score_vec, level_vec, grad_vec, types_vec, grid_);
  }
...
{
  {
    ...
    Corners corners(
      grid_.n_cols * grid_.n_rows, Corner(0, 0, options_.threshold_shitomasi, 0, 0.0f));
    fd_utils::shiTomasiDetector(
      img_pyr, options_.threshold_shitomasi, options_.border, options_.min_level,
      options_.max_level, corners, grid_, closeness_check_grid_);
    fd_utils::fillFeatures(
      corners, FeatureType::kCorner, mask, options_.threshold_shitomasi, max_n_features, px_vec,
      score_vec, level_vec, grad_vec, types_vec, grid_);
  }
  ...
  if (max_features > 0) {
    ...
    Corners corners_fast(
      grid_.n_cols * grid_.n_rows, Corner(0, 0, options_.threshold_primary, 0, 0.0f));
    fd_utils::fastDetector(
      img_pyr, options_.threshold_primary, options_.border, options_.min_level, options_.max_level,
      corners_fast, grid_);
    fd_utils::fillFeatures(
      corners_fast, FeatureType::kCorner, mask, options_.threshold_primary, max_n_features, px_vec,
      score_vec, level_vec, grad_vec, types_vec, grid_);
    ...
    fd_utils::fillFeatures(
      corners_grad, FeatureType::kEdgelet, mask, options_.threshold_secondary, max_features, px_vec,
      score_vec, level_vec, grad_vec, types_vec, grid_);
  }
...
{
  {
    ...
    fd_utils::shiTomasiDetector(
      ...
      options_.max_level, corners, grid_, closeness_check_grid_);
    fd_utils::fillFeatures(
      corners, FeatureType::kMapPoint, mask, options_.threshold_shitomasi, max_n_features, px_vec,
      score_vec, level_vec, grad_vec, types_vec, grid_);
  }
  resetGrid();
}
  ...
  Scores & score_vec, Levels & level_vec, Gradients & grad_vec, FeatureTypes & types_vec)
{
  ...
  cv::threshold(
    mag_level_32f, mag_level_thresholded, options_.threshold_primary, 1.0, cv::THRESH_BINARY_INV);
