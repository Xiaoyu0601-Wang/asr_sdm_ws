...
inline cv::Mat drawHist(
  const std::vector<size_t> & hist, const std::vector<double> & bounds, int width, int height)
{
  ...
  for (size_t i = 0; i < hist_bin_num; i++) {
    ...
    cv::rectangle(
      ...
      cv::Point2f(bin_bl + 0.9 * bin_width, height - bin_height), cv::Scalar(0), cv::FILLED);
    cv::putText(
      histogram, ss.str(), cv::Point2f(bin_bl + 0.1 * bin_width, height / 2.0),
      cv::FONT_HERSHEY_PLAIN, 1.2, cv::Scalar(120), 2);
    cv::putText(
      histogram, std::to_string(hist[i]), cv::Point2f(bin_bl + 0.1 * bin_width, height * 0.8 / 2.0),
      cv::FONT_HERSHEY_PLAIN, 1.2, cv::Scalar(120), 2);
  }
  return histogram;
}
