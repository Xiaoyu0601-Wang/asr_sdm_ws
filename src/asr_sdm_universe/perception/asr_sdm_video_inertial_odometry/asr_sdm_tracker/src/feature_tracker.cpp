...
FeatureTracker::FeatureTracker(
  const FeatureTrackerOptions & options, const DetectorOptions & detector_options,
  ...
  for (size_t frame_index = 0; frame_index < bundle_size_; ++frame_index) {
    ...
    for (size_t track_index = 0; track_index < tracks.size(); ++track_index) {
      ...
      bool success = feature_alignment::alignPyr2D(
        ref_pyr, cur_pyr, options_.klt_max_level, options_.klt_min_level, options_.klt_patch_sizes,
        options_.klt_max_iter, options_.klt_min_update_squared, ref_px_level_0, cur_px_level_0);
      if (success) {
...
size_t FeatureTracker::initializeNewTracks(const FrameBundlePtr & nframe)
{
  ...
  for (size_t frame_index = 0; frame_index < bundle_size_; ++frame_index) {
    ...
    detectors_.at(frame_index)
      ->detect(
        frame->img_pyr_, frame->getMask(), max_n_features, new_px, new_scores, new_levels,
        new_grads, new_types);
...
void FeatureTracker::getNumTrackedAndDisparityPerFrame(
  double pivot_ratio, std::vector<size_t> * num_tracked, std::vector<double> * disparity) const
{
