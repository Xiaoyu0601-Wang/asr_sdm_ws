    ...
    const FrameBundlePtr& frame_bundle)
{
  ...
  // frame_bundle. This matches rpg_svo_pro_open behavior and avoids
  // mismatched ordering/coverage assumptions.
  ...
  // ImuHandler follows upstream contract: newest measurement at front.
  // Sanity check: timestamps must be strictly decreasing.
  for (size_t i = 1; i < imu_measurements.size(); ++i)
  {
    if (!(imu_measurements[i].timestamp_ < imu_measurements[i - 1].timestamp_))
    {
      LOG(ERROR) << "IMU measurements are not strictly time-ordered (expected newest->oldest). i="
                 << i << " t_prev=" << imu_measurements[i - 1].timestamp_
                 << " t=" << imu_measurements[i].timestamp_;
      return false;
    }
  }

  // Additional coverage check (mirrors ImuError::propagation preconditions)
  // to fail fast with context.
  const double t_start = (last_added_frame_stamp_ns_ > 0u)
                           ? static_cast<double>(last_added_frame_stamp_ns_) * 1e-9
                           : current_frame_bundle_stamp;
  const double t_end = current_frame_bundle_stamp;
  const double t_start_adjusted = t_start - imu_handler_->imu_calib_.delay_imu_cam;
  const double t_end_adjusted = t_end - imu_handler_->imu_calib_.delay_imu_cam;
  if (!imu_measurements.empty())
  {
    if (imu_measurements.back().timestamp_ > t_start_adjusted ||
        imu_measurements.front().timestamp_ < t_end_adjusted)
    {
      LOG(ERROR) << std::fixed << std::setprecision(17)
                 << "IMU coverage check failed before backend addStates. "
                 << "oldest=" << imu_measurements.back().timestamp_ << ", "
                 << "newest=" << imu_measurements.front().timestamp_ << ", "
                 << "t_start=" << t_start_adjusted << ", "
                 << "t_end=" << t_end_adjusted << ", "
                 << "raw_t_start=" << t_start << ", raw_t_end=" << t_end;
    }
  }
