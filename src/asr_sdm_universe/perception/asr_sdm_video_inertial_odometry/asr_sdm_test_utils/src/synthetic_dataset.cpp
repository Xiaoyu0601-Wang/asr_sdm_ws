    ...
    cv::Mat* depthmap)
{
  ...
  // make frame
  frame.reset(new Frame(cam_, img, stamp, n_pyramid_levels));
  frame->id_ = img_id;
  ...
  if(depth_fs_.is_open())
  {
    ...
    if(depth_fs_ >> depth_id >> depth_name)
    {
      ...
      if(depthmap)
      {
        ...
        vk::PinholeCamera cam_pinhole(
          static_cast<int>(cam_->imageWidth()),
          static_cast<int>(cam_->imageHeight()),
          cam_->getIntrinsicParameters()(0),
          cam_->getIntrinsicParameters()(1),
          cam_->getIntrinsicParameters()(2),
          cam_->getIntrinsicParameters()(3));
        vk::blender_utils::loadBlenderDepthmap(depth_filename, cam_pinhole, *depthmap);
      }
    }
  }
